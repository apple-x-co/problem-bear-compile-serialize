...
interface ProductPurposeCommandInterface
{
    /**
     ...
     */
    #[DbQuery('product_purpose_add', type: 'row')]
    public function add(
        int $productId,
        int $purposeId,
        int $position,
     ...
     * @param int<1, max> $position
     */
    #[DbQuery('product_purpose_update', type: 'row')]
    public function update(int $id, int $position): void;

    /** @param list<int<1, max>> $aliveIds */
    #[DbQuery('product_purpose_delete_old', type: 'row')]
