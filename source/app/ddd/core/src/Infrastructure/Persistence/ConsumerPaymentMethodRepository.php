...
class ConsumerPaymentMethodRepository extends PaymentMethodRepository implements PaymentMethodRepositoryInterface
{
    public function findById(int $id): PaymentMethod
    {
        $paymentMethod = parent::findById($id);
        if ($paymentMethod->role !== PaymentMethodRole::CONSUMER) {
    ...
    {
        $items = $this->paymentMethodQuery->listForConsumer();
        if (empty($items)) {
            throw new PaymentMethodNotFoundException();
        }
